...
def threadSolver(game, lineList, id_thread):
    lines = list(lineList)
    for key in game:
        for item in lines:
            if (len(set(game[key][item])) != 9):
                errorDict[id_thread].append(str(key) + str(item + 1))
    return 0
...
def checkResult(game, gameI, pid, n_threads):
    for key in gameI:
        ...
        threadLines = list()
        chunk_lines = len(listaI) // n_threads
        r = len(listaI) % n_threads

        begin = 0

        for i in range(n_threads):
            end = begin + chunk_lines
            if r > 0:
                end += 1
                r -= 1
            threadLines[i] = listaI[begin:end]
            begin = end
        ...
        errorCount = 0
        errorList = []
        for key_error in errorDict:
            threadErrors = ', '.join(list(errorDict[key_error]))
            errorCount += len(list(errorDict[key_error]))
            errorStr = ('T%i: %s' % (key_error, threadErrors))
